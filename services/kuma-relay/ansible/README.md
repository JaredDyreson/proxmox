...
Ansible playbook to deploy the `kuma-relay` Rust webhook server behind Caddy on a Debian/Ubuntu VPS.
...
- Built release binary at `target/release/kuma-relay` (run `cargo build --release`)
...
2. Creates the `kuma-relay` system user and `/opt/kuma-relay/`
3. Copies the release binary
4. Installs and starts the systemd unit from `deploy/kuma-relay.service`
